...
class App(customtkinter.CTk):
    ...
    def btn_convertir_c_f_on_click(self):
        numero2 = self.txt_temperatura_c.get()
        numero2 = int(numero2)
        resultadoc = (numero2 * 9/5) + 32
        texto = f"tu resultado en farenheit es {resultadoc}"
        alert("farenheit convertir", texto)
        

    def btn_convertir_f_c_on_click(self):
        numero1 = self.txt_temperatura_f.get()
        numero1 = int(numero1)
        resultado = (numero1 - 32) * 5/9
        texto = f"tu resultado en celcius es {resultado}"
        alert("celcius convertir", texto)

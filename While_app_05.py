...
class App(customtkinter.CTk):
    ...
    def btn_validar_letra_on_click(self):
        salida_prompt = prompt("inserte letra", "inserte letra")
        contador = 0
        while salida_prompt == "U":
                contador += 1
                mensaje = "bien logrado"
                if contador == 2:
                    break
        else: 
            while salida_prompt == "T":
                    contador += 1
                    mensaje = "bien logrado"
                    if contador == 2:
                      break
            else:
                    while salida_prompt == "N":
                        contador += 1
                        mensaje = "bien logrado"
                        if contador == 2:
                           break
                    else: mensaje = "intente de nuevo"
